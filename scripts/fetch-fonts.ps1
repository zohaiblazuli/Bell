...
# ---------------------------------------------------------------- Geist Sans (Vercel, SIL OFL)
# Design System v2 (§6.2) makes Geist the single UI/title sans, replacing SF Pro. Same release zip
# as Geist Mono, so no extra download. 400/500/600 cover the v2 ramp; 700 is kept for emphasis.
Write-Host 'Geist Sans'
$geistSansFaces = @(
  @{ file = 'Geist-Regular.ttf';  out = 'geist-400-normal.woff2' },
  @{ file = 'Geist-Medium.ttf';   out = 'geist-500-normal.woff2' },
  @{ file = 'Geist-SemiBold.ttf'; out = 'geist-600-normal.woff2' },
  @{ file = 'Geist-Bold.ttf';     out = 'geist-700-normal.woff2' }
)
foreach ($f in $geistSansFaces) {
  if (-not (Test-Path (Join-Path $srcDir $f.file))) {
    & $sevenZip e -y $geistZip "geist-font/Geist/ttf/$($f.file)" "-o$srcDir" | Out-Null
  }
  Subset-Face -Source (Join-Path $srcDir $f.file) -OutName $f.out
}
Copy-Item (Join-Path $srcDir 'OFL.txt') (Join-Path $outDir 'geist-OFL.txt') -Force
...
[void]$sb.AppendLine('/*')
[void]$sb.AppendLine(' * Geist      - UI and titles (Design System v2). The single sans voice: --font-ui and --font-disp.')
[void]$sb.AppendLine(' * SF Pro     - legacy, retained only for screens not yet ported to v2. Removed after migration.')
...
Add-Face 'Geist'          '400' 'geist-400-normal.woff2'
Add-Face 'Geist'          '500' 'geist-500-normal.woff2'
Add-Face 'Geist'          '600' 'geist-600-normal.woff2'
Add-Face 'Geist'          '700' 'geist-700-normal.woff2'
